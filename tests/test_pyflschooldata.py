...
class TestImport:
    ...
    def test_import_functions(self):
        ...
        import pyflschooldata as fl
        assert hasattr(fl, 'fetch_enr')
        assert hasattr(fl, 'fetch_enr_multi')
        assert hasattr(fl, 'tidy_enr')
        assert hasattr(fl, 'get_available_years')

    def test_version_exists(self):
        """Package has a version string."""
        import pyflschooldata as fl
        assert hasattr(fl, '__version__')
        assert isinstance(fl.__version__, str)
    ...
    def test_total_enrollment_reasonable(self):
        ...
        if 'is_district' in df.columns and 'grade_level' in df.columns:
            ...
            if len(total_df) > 0:
                ...
                # FL should have ~2.9 million students
...
class TestDataIntegrity:
    ...
    def test_years_within_available_range(self):
        ...
        df = fl.fetch_enr(years['max_year'])
        assert len(df) > 0
