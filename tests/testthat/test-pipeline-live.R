...
test_that("Florida DOE website is accessible", {

  skip_if_offline()
  skip("TODO: Add Florida DOE URL and verify HTTP 200")
})
...
test_that("Can download Florida enrollment data file", {
  skip_if_offline()
...
test_that("Can parse Florida enrollment file", {
  skip_if_offline()
...
test_that("flschooldata data file has expected columns", {
...
test_that("get_raw_enr returns data for valid year", {
  ...
  # Get available years
  years_info <- flschooldata::get_available_years()
  ...
  tryCatch({
    raw <- flschooldata:::get_raw_enr(test_year)
...
test_that("get_available_years returns valid year range", {
  result <- flschooldata::get_available_years()
...
test_that("fetch_enr returns data with no Inf or NaN", {
  ...
  tryCatch({
    ...
    data <- flschooldata::fetch_enr(test_year, tidy = TRUE)
...
test_that("Enrollment counts are non-negative", {
  ...
  tryCatch({
    ...
    data <- flschooldata::fetch_enr(test_year, tidy = FALSE)
...
test_that("tidy=TRUE and tidy=FALSE return consistent totals", {
  ...
  tryCatch({
    ...
    wide <- flschooldata::fetch_enr(test_year, tidy = FALSE)
    tidy <- flschooldata::fetch_enr(test_year, tidy = TRUE)
...
test_that("Cache functions exist and work", {
  ...
  tryCatch({
    path <- flschooldata:::get_cache_path(2024, "enrollment")
